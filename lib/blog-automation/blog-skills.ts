...
이 글의 핵심은 **"사진의 시각적 디테일과 글의 서사(Storyline)가 한 몸처럼 유기적으로 맞물려 돌아가는 현장 스토리 중심의 블로그"**입니다!
정보를 먼저 나열하고 사진을 기계적으로 끼워 넣는 글은 절대 금지합니다.

1. [사진 순서에 따른 자연스러운 서사 여정 (Narrative Journey)]:
   - [IMAGE_1]부터 [IMAGE_${imageDescriptions.length}]까지의 사진 순서가 곧 글의 **'시간적·공간적 이야기의 흐름'**이 되어야 합니다.
   - 예: 산책길 출발/입구([IMAGE_1]) → 길가에서 발견한 첫 피사체/밤송이([IMAGE_2]) → 숲속 그늘/꽃([IMAGE_3]) → 쉼터/전망([IMAGE_4]) → 마무리 소회([IMAGE_5])
   
2. [사진 밀착형 에피소드 & 디테일 묘사]:
   - **사진 직전**: 그 장면을 찍게 된 상황, 시선이 머문 이유, 현장의 소리와 공기 (예: "발밑에서 바스락 소리가 나 고개를 숙여보니...")
   - **[IMAGE_N] 단독 줄 배치**
   - **사진 직후**: 사진 속 구체적 피사체(초록 밤송이의 가시, 청초한 파란 달개비 꽃, 싱그러운 나뭇잎 등)를 생생하게 묘사하고, 그 피사체에서 자연스럽게 건강 지식/정보/인사이트로 물 흐르듯 연결!
   - (예: "사진 속 귀여운 밤송이를 가만히 들여다보니 절로 미소가 지어지더라고요. 이렇게 길가의 작은 생명을 관찰하며 걷는 몰입의 순간이 바로 뇌의 코르티솔(스트레스 호르몬)을 낮추고...")

3. [1인칭 생생한 현장 보이스]:
   - "오늘 아침 제가 직접 걸으며 담아온 풍경", "실제 현장에서 느낀 생생한 기운"을 바탕으로 작성.
   - 독자가 글쓴이와 함께 산책로를 나란히 걸으며 대화하는 듯한 따뜻하고 유기적인 호흡을 유지할 것.
4. 모든 사진([IMAGE_1] ~ [IMAGE_${imageDescriptions.length}])을 빠짐없이 이야기의 이정표로 활용할 것.`;
  }
