...
${imageUrls?.length ? `9. [★ 사진의 유기적 맥락 배치 (최우선 준수)]:
- 총 ${imageUrls.length}장의 사진이 제공됩니다:
${(imageDescriptions || []).map((d, i) => `  * [IMAGE_${i + 1}]: ${d}`).join('\n')}
- **절대로 [IMAGE_1], [IMAGE_2], [IMAGE_3] 번호 순서대로 기계적으로 나열하지 마십시오!**
- 각 사진의 분석된 피사체(예: 아침 숲길 전경, 파란 닭의장풀 꽃 근접샷, 산책로 흙길 등)를 깊이 이해하고, 글의 스토리 서사(도입부 전경 -> 본문 중 야생화 발견의 설렘 -> 관절에 좋은 흙길 걷기 팁 -> 마무리)와 **가장 완벽하게 부합하는 문맥 위치에 유기적으로 배치**하십시오.
- 본문 텍스트에서 해당 사진 속 피사체의 구체적인 모습과 감상을 자연스럽게 언급하여, 독자가 실제 현장에 있는 것처럼 생생한 몰입감을 느끼게 하십시오.` : ''}
